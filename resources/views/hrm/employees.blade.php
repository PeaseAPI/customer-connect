            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <form method="POST" :action="editEmployee ? '{{ route('hrm.employees.update', ['id' => 'EID']) }}'.replace('EID', editEmployee.id) : '{{ route('hrm.employees.store') }}'">
                    ...
                    <div class="space-y-4">
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Department</label><input name="department" @if(old('department')) value="{{ old('department') }}" @else :value="editEmployee?.department" @endif class="w-full rounded-lg border {{ $errors->has('department') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
                            @error('department')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Status</label><select name="status" class="w-full rounded-lg border {{ $errors->has('status') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm"><option value="active" :selected="(editEmployee?.status ?? '{{ old('status', 'active') }}') === 'active'">Active</option><option value="inactive" :selected="(editEmployee?.status ?? '{{ old('status', 'active') }}') === 'inactive'">Inactive</option></select>
