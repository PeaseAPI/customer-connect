...
<div x-data="{ showModal: {{ $errors->any() ? 'true' : 'false' }}, editEmployee: null }">
    ...
    <form method="GET" action="{{ route('hrm.employees') }}" class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative flex-1 min-w-[200px]">
            ...
            <input name="search" type="text" value="{{ request('search') }}" placeholder="Search employees..." class="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-3 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
        </div>
        ...
        <a href="{{ route('hrm.employees') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
        @endif
    </form>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <form method="POST" :action="editEmployee ? '{{ route('hrm.employees.update', ['id' => 'EID']) }}'.replace('EID', editEmployee.id) : '{{ route('hrm.employees.store') }}'">
                    ...
                    @csrf
                    <div class="space-y-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Full Name *</label><input name="name" @if(old('name')) value="{{ old('name') }}" @else :value="editEmployee?.name" @endif class="w-full rounded-lg border {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Email *</label><input name="email" type="email" @if(old('email')) value="{{ old('email') }}" @else :value="editEmployee?.email" @endif class="w-full rounded-lg border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('email')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Department *</label><select name="department_id" class="w-full rounded-lg border {{ $errors->has('department_id') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required><option value="">-- Select --</option>@foreach($departments ?? [] as $department)<option value="{{ $department['id'] }}" :selected="(editEmployee?.employeeDetail?.department_id ?? '{{ old('department_id') }}') == {{ $department['id'] }}">{{ $department['department_name'] }}</option>@endforeach</select>
                            @error('department_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Designation *</label><select name="designation_id" class="w-full rounded-lg border {{ $errors->has('designation_id') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required><option value="">-- Select --</option>@foreach($designations ?? [] as $designation)<option value="{{ $designation['id'] }}" :selected="(editEmployee?.employeeDetail?.designation_id ?? '{{ old('designation_id') }}') == {{ $designation['id'] }}">{{ $designation['designation_name'] }}</option>@endforeach</select>
                            @error('designation_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Status</label><select name="status" class="w-full rounded-lg border {{ $errors->has('status') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm"><option value="active" :selected="(editEmployee?.status ?? '{{ old('status', 'active') }}') === 'active'">Active</option><option value="deactive" :selected="(editEmployee?.status ?? '{{ old('status', 'active') }}') === 'deactive'">Inactive</option></select>
                            @error('status')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div x-show="!editEmployee"><label class="block text-sm font-medium text-gray-700 mb-1">Password *</label><input name="password" type="password" class="w-full rounded-lg border {{ $errors->has('password') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
