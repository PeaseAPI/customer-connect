...
<div x-data="{ showModal: {{ $errors->any() ? 'true' : 'false' }}, editPayment: null }">
    ...
    <form method="GET" action="{{ route('finance.payments') }}" class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative flex-1 min-w-[200px]">
            ...
            <input name="search" type="text" value="{{ request('search') }}" placeholder="Search payments..." class="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-3 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
        </div>
        ...
        @if(request('search'))
        <a href="{{ route('finance.payments') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
        @endif
    </form>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <form method="POST" action="{{ route('finance.payments.store') }}">
                    @csrf
                    <div class="space-y-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Invoice Number *</label><input name="invoice_number" @if(old('invoice_number')) value="{{ old('invoice_number') }}" @else :value="editPayment?.invoice_number" @endif class="w-full rounded-lg border {{ $errors->has('invoice_number') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('invoice_number')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Amount *</label><input name="amount" type="number" step="0.01" @if(old('amount')) value="{{ old('amount') }}" @else :value="editPayment?.amount" @endif class="w-full rounded-lg border {{ $errors->has('amount') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Payment Date</label><input name="paid_on" type="date" @if(old('paid_on')) value="{{ old('paid_on') }}" @else :value="editPayment?.paid_on" @endif class="w-full rounded-lg border {{ $errors->has('paid_on') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
