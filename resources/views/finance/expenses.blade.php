...
<div x-data="{ showModal: {{ $errors->any() ? 'true' : 'false' }}, editExpense: null }">
    <div class="mb-6 flex items-center justify-between">
        ...
        <button @click="showModal = true; editExpense = null" class="inline-flex items-center gap-2 rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white hover:bg-primary-500">+ New Expense</button>
    </div>
    ...
    <form method="GET" action="{{ route('finance.expenses') }}" class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative flex-1 min-w-[200px]">
            ...
            <input name="search" type="text" value="{{ request('search') }}" placeholder="Search expenses..." class="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-3 text-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
        ...
        <a href="{{ route('finance.expenses') }}" class="text-sm text-bodytext hover:text-ink">Clear</a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-xl border border-stroke overflow-hidden">
        <table class="min-w-full divide-y divide-stroke">
            <thead class="bg-surface"><tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-bodytext uppercase">Item</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-bodytext uppercase">Category</th>
            ...
            <tbody class="divide-y divide-stroke">
                ...
                <tr class="hover:bg-surface">
                    <td class="px-6 py-4 text-sm text-ink">{{ $expense['item_name'] ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm text-bodytext">{{ $expense['category'] ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm text-ink">${{ number_format($expense['amount'] ?? 0, 2) }}</td>
                    <td class="px-6 py-4 text-sm text-bodytext">{{ $expense['purchase_date'] ?? '-' }}</td>
                    ...
                    <td class="px-6 py-4 text-right text-sm space-x-2">
                        <button @click="editExpense = {{ json_encode($expense) }}; showModal = true" class="text-primary-500 hover:text-primary-800">Edit</button>
                ...
                @endforeach
                @if(count($expenses) === 0)
                <tr><td colspan="6" class="px-6 py-12 text-center text-sm text-bodytext">No expenses found</td></tr>
                @endif
            </tbody>
        </table>
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                <h3 class="text-lg font-semibold text-ink mb-4" x-text="editExpense ? 'Edit Expense' : 'New Expense'"></h3>
                <form method="POST" :action="editExpense ? '{{ route('finance.expenses.update', ['id' => 'EID']) }}'.replace('EID', editExpense.id) : '{{ route('finance.expenses.store') }}'">
                    ...
                    @csrf
                    <div class="space-y-4">
                        <div><label class="block text-sm font-medium text-ink mb-1">Item Name *</label><input name="item_name" @if(old('item_name')) value="{{ old('item_name') }}" @else :value="editExpense?.item_name" @endif class="w-full rounded-lg border {{ $errors->has('item_name') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('item_name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-ink mb-1">Category</label><select name="category_id" class="w-full rounded-lg border {{ $errors->has('category_id') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm"><option value="">-- None --</option>@foreach($expenseCategories ?? [] as $category)<option value="{{ $category['id'] }}" :selected="(editExpense?.category_id ?? '{{ old('category_id') }}') == {{ $category['id'] }}">{{ $category['category_name'] }}</option>@endforeach</select>
                            @error('category_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-ink mb-1">Amount *</label><input name="amount" type="number" step="0.01" @if(old('amount')) value="{{ old('amount') }}" @else :value="editExpense?.amount" @endif class="w-full rounded-lg border {{ $errors->has('amount') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('amount')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-ink mb-1">Purchase Date</label><input name="purchase_date" type="date" @if(old('purchase_date')) value="{{ old('purchase_date') }}" @else :value="editExpense?.purchase_date" @endif class="w-full rounded-lg border {{ $errors->has('purchase_date') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
