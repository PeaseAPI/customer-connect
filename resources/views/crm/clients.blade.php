...
<div x-data="{ showModal: {{ $errors->any() ? 'true' : 'false' }}, editClient: null }">
    ...
    <form method="GET" action="{{ route('crm.clients') }}" class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative flex-1 min-w-[200px]">
            ...
            <input name="search" type="text" value="{{ request('search') }}" placeholder="Search clients..." class="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-3 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
        </div>
        ...
        @if(request('search'))
        <a href="{{ route('crm.clients') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
        @endif
    </form>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <form method="POST" :action="editClient ? '{{ route('crm.clients.update', ['id' => 'CID']) }}'.replace('CID', editClient.id) : '{{ route('crm.clients.store') }}'">
                    ...
                    @csrf
                    <div class="space-y-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Name *</label><input name="name" @if(old('name')) value="{{ old('name') }}" @else :value="editClient?.name" @endif class="w-full rounded-lg border {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Email *</label><input name="email" type="email" @if(old('email')) value="{{ old('email') }}" @else :value="editClient?.email" @endif class="w-full rounded-lg border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('email')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Industry</label><input name="industry" @if(old('industry')) value="{{ old('industry') }}" @else :value="editClient?.industry" @endif class="w-full rounded-lg border {{ $errors->has('industry') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
                            @error('industry')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div x-show="!editClient"><label class="block text-sm font-medium text-gray-700 mb-1">Password *</label><input name="password" type="password" class="w-full rounded-lg border {{ $errors->has('password') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
