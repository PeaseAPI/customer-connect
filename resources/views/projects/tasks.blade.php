...
<div x-data="{ showModal: {{ $errors->any() ? 'true' : 'false' }}, editTask: null }">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-ink">Tasks</h1>
        <button @click="showModal = true; editTask = null" class="inline-flex items-center gap-2 rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white hover:bg-primary-500">+ New Task</button>
    </div>
    ...
    <form method="GET" action="{{ $projectId ? route('projects.tasks', $projectId) : route('tasks.index') }}" class="mb-4 flex flex-wrap items-center gap-3">
        <div class="relative flex-1 min-w-[200px]">
            ...
            <input name="search" type="text" value="{{ request('search') }}" placeholder="Search tasks..." class="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-3 text-sm focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
        </div>
        ...
        <a href="{{ $projectId ? route('projects.tasks', $projectId) : route('tasks.index') }}" class="text-sm text-bodytext hover:text-ink">Clear</a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-xl border border-stroke overflow-hidden">
        <table class="min-w-full divide-y divide-stroke">
            <thead class="bg-surface"><tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-bodytext uppercase">Title</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-bodytext uppercase">Status</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-bodytext uppercase">Priority</th>
            ...
            <tbody class="divide-y divide-stroke">
                ...
                <tr class="hover:bg-surface">
                    <td class="px-6 py-4 text-sm font-medium text-ink">{{ $task['title'] ?? '-' }}</td>
                    ...
                    <td class="px-6 py-4"><span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ ($task['priority'] ?? '') === 'high' ? 'bg-red-100 text-red-800' : (($task['priority'] ?? '') === 'medium' ? 'bg-amber-100 text-amber-800' : 'bg-surface text-ink') }}">{{ $task['priority'] ?? 'low' }}</span></td>
                    <td class="px-6 py-4 text-sm text-bodytext">{{ $task['due_date'] ?? '-' }}</td>
                    <td class="px-6 py-4 text-right text-sm space-x-2">
                        <button @click="editTask = {{ json_encode($task) }}; showModal = true" class="text-primary-500 hover:text-primary-800">Edit</button>
                ...
                @endforeach
                @if(count($tasks) === 0)
                <tr><td colspan="5" class="px-6 py-12 text-center text-sm text-bodytext">No tasks found</td></tr>
                @endif
            </tbody>
        </table>
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                <h3 class="text-lg font-semibold text-ink mb-4" x-text="editTask ? 'Edit Task' : 'New Task'"></h3>
                <form method="POST" :action="editTask ? '{{ route('tasks.update', ['id' => 'TID']) }}'.replace('TID', editTask.id) : '{{ route('tasks.store') }}'">
                    ...
                    @csrf
                    <div class="space-y-4">
                        <div><label class="block text-sm font-medium text-ink mb-1">Title *</label><input name="title" @if(old('title')) value="{{ old('title') }}" @else :value="editTask?.title" @endif class="w-full rounded-lg border {{ $errors->has('title') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm" required>
                            @error('title')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-ink mb-1">Status</label><select name="status" class="w-full rounded-lg border {{ $errors->has('status') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm"><option value="pending" :selected="(editTask?.status ?? '{{ old('status', 'pending') }}') === 'pending'">Pending</option><option value="in_progress" :selected="(editTask?.status ?? '{{ old('status', 'pending') }}') === 'in_progress'">In Progress</option><option value="completed" :selected="(editTask?.status ?? '{{ old('status', 'pending') }}') === 'completed'">Completed</option></select>
                            @error('status')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-ink mb-1">Priority</label><select name="priority" class="w-full rounded-lg border {{ $errors->has('priority') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm"><option value="low" :selected="(editTask?.priority ?? '{{ old('priority', 'low') }}') === 'low'">Low</option><option value="medium" :selected="(editTask?.priority ?? '{{ old('priority', 'low') }}') === 'medium'">Medium</option><option value="high" :selected="(editTask?.priority ?? '{{ old('priority', 'low') }}') === 'high'">High</option></select>
                            @error('priority')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-ink mb-1">Due Date</label><input name="due_date" type="date" @if(old('due_date')) value="{{ old('due_date') }}" @else :value="editTask?.due_date" @endif class="w-full rounded-lg border {{ $errors->has('due_date') ? 'border-red-500' : 'border-gray-300' }} px-3 py-2 text-sm">
